...
def production_plan(df):
    ...
    df_kitchen = df[
        (df['done'] == False) & (df['category'].isin(['sandwich', 'roller_grill', 'side']))
    ]
    df_chicken = df[
        (df['done'] == False) & (df['category'].isin(['chicken', 'appetizer']))
    ]
    ...
    df_done = df[df["item_id"].isin(st.session_state.done_items)]
